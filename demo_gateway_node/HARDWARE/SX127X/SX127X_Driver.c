...
{
    SX127X_RESET_OUTPUT(Bit_RESET);
    delay_ms(2);
    SX127X_RESET_OUTPUT(Bit_SET);
    delay_ms(15);
	//printf ("sx1278复位成功\r\n");
}
/**
...
//    SX127X_DIO2_INPUT();
    ...
    SX127X_TXE_OUTPUT(Bit_RESET);
    SX127X_RXE_OUTPUT(Bit_RESET);
}
/**
...
void SX127XWriteRxTx( bool txEnable )
{
    ...
    {
        SX127X_RXE_OUTPUT(Bit_RESET);
        SX127X_TXE_OUTPUT(Bit_SET);
    }
    else  //为假，为RX
    {
        SX127X_RXE_OUTPUT(Bit_SET);
        SX127X_TXE_OUTPUT(Bit_RESET);
    }
}
/**
...
tSX127xError SX127X_Lora_init(void)
{
    ...
    SX127X_InitIo();              								//IO初始化	
    SX127X_Reset();           								//复位IO初始化	
    //SX127X_SPI_Init();     									//SPI外设初始化
	...
	}//else printf("SX1278寄存器读写检查成功:%x\r\n",test); 
	
    SX127X_Write( REG_LR_DIOMAPPING2, RFLR_DIOMAPPING2_DIO4_01);
    SX127X_Write(REG_LR_LNA, 0x23);			//LNA最大增益，150%灵敏度
    SX127X_FreqSet(); 
    SX127X_PoutSet();
...
void SX127X_TxPacket(uint8_t*data)
{
    ...
    SX127X_Write(REG_LR_IRQFLAGS, 0xff);								//清除全部中断
    SX127X_Write( REG_LR_IRQFLAGSMASK, ~(RFLR_IRQFLAGS_TXDONE));		//屏蔽除接收中断外的全部中断
    SX127X_Write( REG_LR_DIOMAPPING1, RFLR_DIOMAPPING1_DIO0_01 );		//DIO0映射为接收中断输出
...
void SX127X_StartRx(void)
{
    ...
    SX127X_Write(REG_LR_IRQFLAGS, 0xff);							//sx1278清除全部中断
    SX127X_Write( REG_LR_IRQFLAGSMASK, ~(RFLR_IRQFLAGS_RXDONE));	//屏蔽除接收外全部中断
...
void SX127X_RxPacket(uint8_t*cbuf)
{
    ...
    SX127X_Write(REG_LR_IRQFLAGS, 0xff);					//读完数据清除全部中断
	c[0]=*cbuf;
	for(i=0;i < 10;i++)
	{printf ("读出数据:%x\r\n",c[i]);}
}
...
tSX127xError SX127X_FreqSet(void)
{
	...
	uint32_t freq_reg ;
	u8 test_FRFMSB,test_FRFMID,test_FRFLSB;
    if((G_LoRaConfig.LoRa_Freq > 920000000) || (G_LoRaConfig.LoRa_Freq < 430000000))
        return PARAMETER_INVALID;
    freq_reg = (uint32_t)(G_LoRaConfig.LoRa_Freq / FREQ_STEP);
    test_FRFMSB = 0, test_FRFMID = 0, test_FRFLSB = 0;
    SX127X_StandbyMode();
...
tSX127xError SX127X_PoutSet(void)
{
    ...
    if(true == G_LoRaConfig.MaxPowerOn)	//PA更大功率设置
