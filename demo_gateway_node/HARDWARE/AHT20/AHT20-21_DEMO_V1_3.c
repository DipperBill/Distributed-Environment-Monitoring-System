...
void SDA_Pin_Output_High(void)   //将PB15配置为输出 ， 并设置为高电平， PB15作为I2C的SDA
{
	...
	GPIO_SetBits(aht20_sda_port,aht20_sda_pin);
}
...
void SDA_Pin_Output_Low(void)  //将P15配置为输出  并设置为低电平
{
	...
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB,& GPIO_InitStruct);
	GPIO_ResetBits(aht20_sda_port,aht20_sda_pin);
}
...
{
	GPIO_SetBits(aht20_scl_port,aht20_scl_pin);
}

void SCL_Pin_Output_Low(void) //SCL输出低电平
{
	GPIO_ResetBits(aht20_scl_port,aht20_scl_pin);
}
...
{	
	...
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB,& GPIO_InitStruct);
	GPIO_SetBits(aht20_scl_port,aht20_scl_pin);//输出高电平
	GPIO_SetBits(aht20_sda_port,aht20_sda_pin);//输出高电平
}
...
uint8_t AHT20_RD_Byte(void)//从AHT20读取一个字节
{
    ...
    SCL_Pin_Output_High();		
		Delay_5us();
		a=0;
		if(GPIO_ReadInputDataBit(aht20_sda_port,aht20_sda_pin)) a=1;
		Byte = (Byte<<1)|a;
		SCL_Pin_Output_Low();
		Delay_5us();
	}
...
uint8_t Receive_ACK(void)   //看AHT20是否有回复ACK
{
	...
	SCL_Pin_Output_High();	
	SensorDelay_us(8);	
	/*while((GPIO_ReadInputDataBit(aht20_sda_port,aht20_sda_pin))  && CNT < 100) 
	CNT++;
	if(CNT == 100)
	{
		return 0;
	}*/
	while(GPIO_ReadInputDataBit(aht20_sda_port,aht20_sda_pin))
	{
			CNT++;
			if (CNT>250)
			{
